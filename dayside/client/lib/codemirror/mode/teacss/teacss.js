        ...
        token: function (stream, stack) {
            switch (stack.state) {
                ...
                case "scope_in_js":
                    ...
                    if (stream.match("@ ")) {
                        stack.push("js_line", {jsState: jsMode.startState()});
                        return 'js_line_start';
                    }
                    if (stream.match("@{")) {
                        stack.push("js_block", {jsState: jsMode.startState()});
                ...
                case "rule":
                    ...
                    if (stack.data.string_single) {
                        ...
                        if (stream.peek() == "'") {
                        ...
                        else if (stream.match("url(", false)) {
                            stack.data.inside_url = true;
                        }
                            ...
                            stream.next();
                            stack.pop();
                            stack.push('scope', {scope: scope});
                            return "scope_start";
                        }
                        else if (stream.peek() == ":" && !stack.data.value_part) {
                            stack.data.value_part = true;
                        }
                        else {
