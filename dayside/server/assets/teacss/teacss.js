window.teacss = window.teacss || (function () {
        ...
        (function () {
            function a(a) {
                function l() {
                    if (g && d < a) {
                        var b = g, c = b[0], f = Array.prototype.slice.call(b, 1), m = b.index;
                ...
                var c = {}, d = 0, e = 0, f = -1, g, h, i = null, j = [], k = b;
            ...
            typeof module == "undefined" ? self.queue = a : module.exports = a, a.version = "0.0.2"
        })();
        teacss.queue = queue;
            ...
            getJS: function (ast) {
                ...
                if (this.format == "string") {
                    for (var t = 0; t < ast.selector_tokens.length; t++) {
                        ...
                        switch (token.name) {
                            case "js_inline":
                                selector_string += '(' + token.data.slice(1) + ')';
                                break;
                            case "js_inline_block":
                                selector_string += '(' + token.data.slice(2, -1) + ')';
                                break;
                            case "rule":
                                var data = token.data;
                                if (t == 0 && token.data[0] == "!") {
                                    data = token.data.substring(1);
                                    modifiers = "!";
                                }
                    ...
                    if (child.name == "rule") {
                        if (child.is_block) {
                            ...
                            for (var j = 0; j < chunks.length; j++) {
                                ...
                                switch (chunk.name) {
                                    case "js_inline":
                                        string_list.push('(' + chunk.data.slice(1) + ')');
                                        break;
                                    case "js_inline_block":
                                        string_list.push('(' + chunk.data.slice(2, -1) + ')');
                                        break;
                                    default:
            ...
            rule: function (key, val) {
                if (val && val.constructor && val.call && val.apply) {
                    ...
                    if (this.current)
                        ...
                        this.rules.push({
                            indent: this.indent, getOutput: function () {
        ...
        teacss.token = function (stream, stack) {
            switch (stack.state) {
                ...
                case "js_block":
                    if (stream.match("'")) {
                        stack.push('string_single', {token: "js_code"});
                        return "js_code";
                    }
                    if (stream.match('"')) {
                        stack.push('string_double', {token: "js_code"});
                        return "js_code";
                    }
                    if (stream.match("//")) {
                        stack.push("comment_line", {token: "js_code"});
                        return "js_code";
                    }
                    if (stream.match("/*")) {
                        stack.push("comment", {token: "js_code"});
                        return "js_code";
                    }
                    if (stream.match("@{")) {
                        stack.push("scope");
                        return "scope_start_js";
                    }
                ...
                case "rule":
                    if (stream.match("@{")) {
                        stack.push("js_inline_block");
                        return "js_inline_block";
                    }
                    if (stream.match("@")) {
                        stack.push("js_inline");
                        return "js_inline";
                    }
                    ...
                    if (stack.data.string_single) {
                        ...
                        if (stream.peek() == "(") {
                            ...
                            stack.data.string_single = true;
                        }
                        else if (stream.peek() == '"') {
                            stack.data.string_double = true;
                        }
                        if (stack.data.braces == 0) {
                ...
                case "js_inline_block":
                    if (stream.match("'")) {
                        stack.push('string_single', {token: "js_inline_block"});
                        return "js_inline_block";
                    }
                    if (stream.match('"')) {
                        stack.push('string_double', {token: "js_inline_block"});
                        return "js_inline_block";
                    }
        ...
        teacss.parse = function (code, path) {
            ...
            for (var t = 0; t < tokens.length; t++) {
                ...
                switch (token.name) {
                    ...
                    case "comment":
                    case "comment_line":
                        ast.push(token.name, token.data, pos);
                        break;
                    ...
                    case "js_inline_block":
                        ...
                        if (token.name == "rule" && token.data.indexOf(":") != -1) {
                            ...
                            if (is_block) {
                                ...
                                var value_tokens = [{
                                    name: "rule",
                                    data: token.data.substring(ast.selector_key.length + 1)
                                }];
                        ...
                        if (!is_block) {
                            ast.end = ast.start + ast.selector.length;
                            ast = ast.parent;
                        }
                        break;
                    ...
                    case "js_block_start":
                        ...
                        break;
                    case "js_code":
                        ast.push(token.name, token.data);
                        break;
                    case "js_block_end":
                        ast = ast.parent;
                        break;
...
teacss.image = teacss.functions.image = teacss.image || (function () {
        ...
        constructor.base = "/";
        constructor.getDeferred = function () {
            return deferred;
        };
