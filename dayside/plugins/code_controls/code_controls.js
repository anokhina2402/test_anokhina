...
dayside.ready(function () {
    ...
    dayside.editor.mainPanel.leftSplitter.bind("change", function () {
        teapot.compensateScroll()
    });
    dayside.editor.mainPanel.rightSplitter.bind("change", function () {
        teapot.compensateScroll()
    });
    ...
    options.filter = function (meta) {
        if (meta.name == "js_line") {
            ...
            if (matches && matches[3].toLowerCase().indexOf(options.key.toLowerCase()) != -1) {
                ...
                return {
                    value: matches[5],
                    pre: matches[1] + (matches[2] || "") + matches[3] + matches[4],
                    post: matches[6]
                };
            }
        }
