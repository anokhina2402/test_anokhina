teacss.ui.editableCombo = teacss.ui.combo.extend({
    init: function (o) {
        ...
        var input, me = this;
        this._super($.extend({
            items: [input = teacss.ui.html({
                html: "<input style='padding:5px;width:236px;border:1px solid #777' placeholder='Type to add items...'>"
            })],
            comboWidth: 250,
            labelTpl: "${label}",
            itemTpl: [
...
exports = dayside.plugins.live_preview = teacss.jQuery.Class.extend({
    ...
    init: function () {
        ...
        function tabClosed(tab) {
            tab.editor.getValue = function () {
                return FileApi.cache[tab.options.file]
            };
            tabChanged(tab);
        }
        ...
        ed.bind("codeChanged", function (b, tab) {
            if (me.live_preview) tabChanged(tab)
        });
        ed.toolbar.push(
        ...
        var frame;
        var pageCombo = ui.editableCombo({
            change: function () {
