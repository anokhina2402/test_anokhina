...
class FileApi
{
    ...
    function save()
    {
        ...
        if (!$path) {
            echo "ERROR: Invalid file save path";
            die();
        }
    ...
    function createFile()
    {
        $path = $this->_pathFromUrl(@$_REQUEST['path']);
        if (!$path) {
            echo "ERROR: Invalid create file path";
            die();
        }
        $newFile = @$_REQUEST['newFile'];
        if (!$newFile) {
            echo "ERROR: Invalid file name to create";
            die();
        }
    ...
    function createFolder()
    {
        $path = $this->_pathFromUrl(@$_REQUEST['path']);
        if (!$path) {
            echo "ERROR: Invalid create directory path";
            die();
        }
        $newFolder = @$_REQUEST['newFolder'];
        if (!$newFolder) {
            echo "ERROR: Invalid directory name to create";
            die();
        }
    ...
    function rename()
    {
        ...
        $name = @$_REQUEST['name'];
        if (!$path || !$name) {
            echo "ERROR: Invalid path for rename";
            die();
        }

        $newPath = dirname($path) . "/" . $name;
        if (dirname($newPath) != dirname($path)) {
            echo "ERROR: Subpaths are restricted";
            die();
        }
    ...
    {
        $path = array();
        if (!$_REQUEST['path'] || is_string($_REQUEST['path'])) {
            echo "ERROR: Download path is not defined";
            die();
        }
        ...
        if ($path) {
            if (@$_REQUEST['preload']) {
                ...
                if ($res && substr($res, 0, 4) == 'http') {
                    ...
                    echo "ERROR: Error preloading file";
                    die();
                }
            }
            $title = basename($path);
